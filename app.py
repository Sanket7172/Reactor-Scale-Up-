...
st.markdown(
""" <div class="app-header"> <div class="app-header-title">
Reactor Scale-Up Engineering Studio </div>
...
(
    "Shaft Power",
    f"{get_train_value('total_power_kw'):.2f}",
    "kW",
),

(
    "P/V",
    f"{get_train_value('P_per_V_kW_m3'):.3f}",
    "kW/m³",
),

(
    "Total Q",
    f"{get_train_value('total_Q_m3_h'):.1f}",
    "m³/h",
),

(
    "Q/V",
    f"{get_train_value('Q_per_volume_1_s'):.4f}",
    "s⁻¹",
),
...
(
    "Maximum Re",
    (
        f"{get_train_value('maximum_reynolds', None):,.0f}"
        if get_train_value(
            "maximum_reynolds",
            None,
        ) is not None
...
for reactor in reactors:
    ...
    c1.metric(
        "Q/V",
        f"{safe_float(train.get('Q_per_volume_1_s')):.4f} s⁻¹",
    )
...
if len(reactors) < 2:
    ...
    reference_qv = safe_float(
        reference_train.get(
            "Q_per_volume_1_s"
        )
    )

    target_qv = safe_float(
        target_train.get(
            "Q_per_volume_1_s"
        )
    )
...
try:
    ...
    st.download_button(
        ...
        data=pdf_bytes,
        file_name="reactor_scaleup_engineering_report.pdf",
